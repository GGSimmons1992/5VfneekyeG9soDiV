...
def reScoreBasedOnStar(df,starIndex,query,model, job_title_embeddings):
    starTitle = df.loc[starIndex,'job_title']
    starQuery = f"{starTitle} {query}"
    query_embedding = model.encode(starQuery)
    queryArray = query_embedding.reshape(1,-1)

    similarities = cosine_similarity(queryArray, job_title_embeddings)
    df['fit'] = np.array(similarities).reshape(-1,1)
...
def main():
    ...
    if starDecision == 'y':
        ...
        candidates = reScoreBasedOnStar(candidates,starIndex,query,model, job_title_embeddings)
